...
const tileSizeInput = document.getElementById(
  "tileSizeInput",
) as HTMLInputElement;
...
processBtn.onclick = async () => {
  ...
  reader.onload = (e) => {
    const img = new Image();
    img.onload = () => {
      tileData = new TileMapData(img, tileSize);
      const tiles = tileData.GetTiles();
      const cols = (tileData as any).cols;
      const rows = (tileData as any).rows;

      updateTileNeighbors(tiles, tileSize);

      // Display all tile images on screen
      const tileGallery =
        document.getElementById("tileGallery") || document.createElement("div");
      ...
      alert(
        `Created ${tiles.length} tiles. Please check the console for details.`,
      );

      //Zikria Test data
      // Test for SetTileData
      tileData.SetTileData(104, {
        Description: "Bottom Well Tile",
        TileRequirements: "Must be used with tile 92",
      });

      console.log(tileData.GetTiles()[104]);
    };
