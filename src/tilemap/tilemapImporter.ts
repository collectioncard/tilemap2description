...
export class TileMapData {
  ...
  public SetTileData(
    tileId: number,
    data: Partial<Omit<Tile, "TileID"> & { image?: HTMLCanvasElement }>,
  ): void {
    const tile = this.tiles.find((t) => t.TileID === tileId);
    if (!tile) throw new Error(`Tile with ID ${tileId} not found`);
  ...
  return new Promise<{
    ...
    img.onload = () => {
      ...
      for (let y = 0; y < tileSize; y++) {
        left.set(
          imageData.slice((y * tileSize + 0) * 4, (y * tileSize + 1) * 4),
          y * 4,
        );
        right.set(
          imageData.slice(
            (y * tileSize + (tileSize - 1)) * 4,
 ...
 */
export async function updateTileNeighbors(
  tiles: Tile[],
  tileSize: number,
): Promise<void> {
  ...
  for (const tile of tiles) {
    edgesMap.set(
      tile.TileID,
      await getEdgePixels(tile.image.toDataURL(), tileSize),
    );
  }
